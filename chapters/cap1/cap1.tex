...
\begin{document}
	...
	\newpage
	\subsection{Complementos sobre la energía}
	\marginnote{Si se deseara hacer el análisis de estabilidad e inestabilidad para potenciales que sean funciones de varias variables es necesario utilizar \textcolor{blue}{teoría de optimización en funciones de variables variables} según sea el caso. A continuación se expresa el \textcolor{red}{Método de la Matriz Hessiana para n variables} para un punto $\vec{r}_{eq}$: \\ 
		\\
		La matriz Hessiana para una función V se forma con las siguientes entradas: \\ 
		\begin{equation*} 
			H_{ij} = \left . \frac{\partial^2 V}{\partial x_i \partial x_j} \right|_{\vec{r} \: = \: \vec{r}_{eq}}  ; i,j = 0,...,n.
		\end{equation*}
					
		El criterio para la concavidad de la función V usando dicha matriz se basa en el uso de los determinantes ($\Delta$) de las matrices menores de la matriz Hessiana ($h_i$): \\ 

		$\bullet$ Si $\Delta h_{i} > 0$  con $m=0,1,..,n$ entonces es un mínimo local de V. \\

		$\bullet$ Si $\Delta h_{2m+1} < 0 $ ; $m=0,1,..,n$ y $\Delta h_{2m} > 0 $ con $m=0,1,..,n$ entonces es un máximo local de V. \\

		$\bullet$ Cualquier otro patrón de los determinantes de las menores corresponde a un punto de silla. \\

		$\bullet$ Si  $\Delta h_{n} = 0$, el estudio no es concluyente y hay que hacer un análisis particular. Se aclara que $h_{n} = H$
		}
	...
	\includepdf[pages={13-53},templatesize = {8 in}{8.5 in} ]{AMeca.pdf}
	...
	\begin{marginfigure}
		\begin{figure}[H]
			\includegraphics[width=5.8cm]{sismangular1.pdf}
	...
	\begin{marginfigure}
		\begin{figure}[H]
			\includegraphics[width=5.8cm]{sismangular2.pdf}
	...
	\includepdf[pages={135-152},templatesize = {8 in}{8.5 in} ]{AMeca.pdf}
	...
	\subsection{Problemas resueltos}
	\includepdf[pages={164-184},templatesize = {8 in}{8.5 in} ]{AMeca.pdf}
